...
#include "../../Advanced-Data-Structures/Segment-Tree/segTree-with-node-structure/sum_segTree_lazy_prop.cpp"

// Test prime_sieve Small
TEST(SumSegTreeTest, BasicSumTest)
{
  std::vector<int> arr = {10, 1, 5, 13, 20, 9, 3, 15, 16, 17, 18};
  int n = arr.size();
  SumSegTree<int> *segtree = SumSegTree<int>::create(arr);
...
TEST(SumSegTreeTest, UpdateWithSumTest)
{
  std::vector<int> arr = {10, 1, 5, 13, 20, 9, 3, 15, 16, 17, 18};
  int n = arr.size();
  SumSegTree<int> *segtree = SumSegTree<int>::create(arr);
