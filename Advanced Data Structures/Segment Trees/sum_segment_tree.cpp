...
public:
    ...
    SumSegmentTree *leftChild, *rightChild;
    T1 nodeSum;

    /* Constructs a segment tree from the array passed*/
    SumSegmentTree(T1 nodeLeft, T1 nodeRight, vector<T1>& arr){
        this -> nodeLeft = nodeLeft;
        this -> nodeRight = nodeRight;
        this -> leftChild = this -> rightChild = nullptr;

        if( isLeaf() ){
            nodeSum = arr[nodeLeft];
        }
        else{
            T1 mid = (nodeRight - nodeLeft) / 2 + nodeLeft;
            this -> leftChild = new SumSegmentTree(nodeLeft, mid, arr);
            this -> rightChild = new SumSegmentTree(mid + 1, nodeRight, arr);
            this -> recalculate();
        }
    }
    ...
    void recalculate(){
        if( isLeaf() ) return;
        nodeSum = leftChild -> nodeSum + rightChild -> nodeSum;
    }
...
int main(){
    ...
    SumSegmentTree segTree(0ll, n - 1, arr); 
